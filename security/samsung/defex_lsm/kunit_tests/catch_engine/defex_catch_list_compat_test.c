...
static void get_local_syscall_compat_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_PTR_EQ(test, (const struct local_syscall_struct *)NULL,
		get_local_syscall_compat(__NR_compat_syscalls));
	KUNIT_EXPECT_PTR_EQ(test, (const struct local_syscall_struct *)NULL,
		get_local_syscall_compat(__NR_compat_syscalls + 1));
